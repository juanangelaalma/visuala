...
import { mapIneligibleWebhookEvent, parseWebhookFulfillmentOutcome } from "./supabase-billing-webhook-repositories";
import { mapAttempt, mapPayment, mapPaymentMethod, parseCheckoutActions } from "./supabase-billing-repositories";
...
describe("billing Supabase mappings", () => {
  it("maps database rows to domain models", () => {
    ...
    expect(mapAttempt({ id: "attempt", billing_payment_id: "payment", payment_method_id: "method", provider_mapping_id: "mapping", attempt_number: 1, provider: "xendit", environment: "test", mapping_version: 1, provider_method_type: "QR_CODE", provider_channel_code: "QRIS", mapping_config: {}, provider_reference: "reference", provider_idempotency_key: "key", provider_payment_id: null, status: "creating", actions: [], raw_provider_status: null, failure_category: null, expires_at: null, last_reconciled_at: null, completed_at: null, created_at: "created", updated_at: "updated" })).toMatchObject({ billingPaymentId: "payment", providerMethodType: "QR_CODE", providerChannelCode: "QRIS", mappingConfig: {}, providerReference: "reference", actions: [] });
  });

  it("rejects malformed persisted billing values", () => {
    expect(() => parseCheckoutActions([{ type: "redirect", url: 42, expiresAt: null }])).toThrow("Invalid persisted checkout actions");
    expect(() => parseWebhookFulfillmentOutcome("unexpected")).toThrow("Invalid persisted webhook outcome");
  });
