...
const mocks = vi.hoisted(() => ({ services: vi.fn(), checkout: vi.fn(), refresh: vi.fn(), redirect: vi.fn() }));
...
import { createBillingCheckoutAction, refreshBillingPaymentAction } from "./billing-actions";
...
function services(user: unknown = { id: "user-1" }) {
  return { config: { checkoutEnabled: true, qrisEnabled: true, virtualAccountEnabled: false }, authProvider: { getCurrentUser: vi.fn().mockResolvedValue(user) }, checkout: {}, payments: {} };
}
