...
class App {
  ...
  loadAssets(): Promise<void> {
    ...
    return new Promise(resolve => {
      ...
      const smaaAssets: NonNullable<App['assets']['smaa']> = {};
      assets.smaa = smaaAssets;

      searchImage.addEventListener('load', function () {
        smaaAssets.search = this;
        manager.itemEnd('smaa-search');
      });

      areaImage.addEventListener('load', function () {
        smaaAssets.area = this;
...
export const hyperspeedPresetThree = {
...
const Hyperspeed: FC<HyperspeedProps> = ({ effectOptions = DEFAULT_EFFECT_OPTIONS }) => {
  ...
  return <div ref={hyperspeed} className="absolute inset-0 h-full w-full overflow-hidden [&_canvas]:h-full [&_canvas]:w-full" />;
};
