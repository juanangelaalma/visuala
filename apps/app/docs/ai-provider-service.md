...
Upload an authenticated `multipart/form-data` request with an `image` field to `POST /api/ai/assets`. The route validates the image, writes it to private object storage, and registers ownership in the asset repository. Keep the returned `id` as the `assetId`.
...
## Video-flow independence and removal

The AI provider service does not depend on the old video flow, Atlas, credits, or worker modules. Its contracts, service factory, adapters, asset storage, usage storage, configuration, and SQL tables can remain when the old video flow is removed. Migrated routes may still save their results through video-flow repositories after the AI call; that caller-side persistence is outside the provider service.

To remove the old video flow, delete its routes, application modules, infrastructure modules, and worker code independently from its database history. Keep applied migration files unchanged. Keep `domain/ai-service`, `application/ai-service`, `infrastructure/ai-service`, `scripts/ai-service*`, `ai_assets`, `ai_service_operations`, and `ai_service_attempts`.

Before dropping deployed `ai_projects`, `ai_scenes`, or `ai_generations` tables, search application code, database functions, policies, foreign keys, and operational jobs for dependencies. Review the data-loss and rollback impact. Drop the tables only through a new timestamped migration after that review. Search retained source files for old module and table names, then run the AI service tests and `ai:check-config` before deployment.
