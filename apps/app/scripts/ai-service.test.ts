...
function commandFixture(environment: Record<string, string> = { AI_SMOKE_USER_ID: "user-1", AI_SMOKE_ASSET_ID: "asset-1" }) {
  ...
  let id = 0;
  const result = { requestId: "provider-request", profileId: "primary", provider: "google", model: "gemini-test" };
  const dependencies: AIServiceCommandDependencies = {
    ...
    createAIService: () => ({
      generateText: async (request) => { events.push("generateText"); requestIds.push(request.requestId); return result; },
      generateStructured: async (request) => { events.push("generateStructured"); requestIds.push(request.requestId); return { ...result, data: { status: "ok" } }; },
    }),
