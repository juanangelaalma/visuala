...
2. **`ai_assets` stays on Cloudflare R2.** The PRD asks for one canonical store per asset, not one store for the whole product. No video asset is ever written to R2, so nothing is duplicated. Migrating the pre-existing `ai_assets` flow off R2 is a separate, unnecessary change in this plan.
